...
def crawl_all(year, start_day, end_day, worker_id):
    db_name = f'notam_all_{year}_worker_{worker_id}.db'
    ...
    for day_num in range(start_day, end_day + 1):
        ...
        for data_source in ['D', 'I']:  # 둘 다 시도
            payload = {
                'sch_inorout': data_source,
                'sch_airport': '',  # 공항 필터 제거!
    ...
    conn.close()
    print(f"[Worker {worker_id}] {year}: {total_inserted} records")
...
if __name__ == '__main__':
    ...
    crawl_all(year, start_day, end_day, worker_id)
