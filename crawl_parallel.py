...
def crawl_date_range(year, start_day, end_day, worker_id):
    """특정 날짜 범위 크롤링"""
    db_name = f'notam_{year}_worker_{worker_id}.db'
    ...
    for day_num in range(start_day, end_day + 1):
        ...
        for data_source in ['D']:  # 국내만 (국제는 중복)
            payload = {
            ...
            try:
                ...
                print(f"[W{worker_id}] {date_str}: Error - {e}")
            
            time.sleep(0.1)
    
    conn.close()
    print(f"[Worker {worker_id}] Completed: {total_inserted} records saved to {db_name}")

if __name__ == '__main__':
    if len(sys.argv) < 4:
        print("Usage: python crawl_parallel.py <year> <start_day> <end_day> <worker_id>")
        sys.exit(1)
    ...
    crawl_date_range(year, start_day, end_day, worker_id)
